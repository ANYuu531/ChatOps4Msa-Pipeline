...
public class LLMService {
    ...
    public LLMService(Environment env, CapabilityGenerator capabilityGenerator, CapabilityConfigLoader configLoader) {
        ...
        if (embeddingUrl == null || embeddingUrl.isBlank()) embeddingUrl = EmbeddingClient.deriveUrl(OPENAI_API_URL);
        this.embeddingClient = new EmbeddingClient(embeddingUrl, OPENAI_API_KEY,
                env.getProperty("openai.api.embedding-model", "text-embedding-3-small"));
...
//        System.err.println(INTENT_CLASSIFICATION_AND_ENTITY_EXTRACTION_FILE);
    ...
    /**
     * Embeds texts for the report Q&amp;A's semantic retrieval and routing. Delegates to
     * {@link EmbeddingClient}; {@code null} on any failure (the callers fall back to
     * lexical retrieval).
     */
