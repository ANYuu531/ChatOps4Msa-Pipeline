...
**信心判定**：最高分 ≥ `threshold`（預設 0.58）且領先第二名 ≥ `margin`（0.04），或最高分 ≥ `high`（0.85）。信心不足 → LLM planner。
（第二次校準，遮名後：33 句 33 對、有把握且對 22、有把握但錯 0、最低正確分數 0.541；門檻設在正確分數帶下緣。其中 5 句已被加進例句，真 hold-out 是 28 句。）
