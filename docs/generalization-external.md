...
| `loadgenerator → frontend` | **邊對，但作者的出處寫錯了**（見下），標註者也沒標。當時工具也畫不出來（host 在 Dockerfile 的 `ENTRYPOINT` 命令列參數裡）——**2026-09-26 的規則 14 之後畫出來了**，因為 `loadgenerator.yaml:82` 的 `FRONTEND_ADDR: "frontend:80"` 是寫在該 workload 自己 env 裡的字面位址（§1.8） |
...
> **40 條「只有標註者有」的邊裡，沒有一條是作者漏標的。** 它們分成命名問題（17 條）、部署變體（5 條，其實在 truth 裡）、以及標註者的判斷錯誤（18 條）。17 ＋ 5 ＋ 18 ＝ 40。而 **61 條**「只有作者有」的邊裡，**有兩組指出了 truth 真正的弱點**（§3.6）。
...
61 條「只有作者有」的邊裡，大部分是標註者漏抓（沒讀 nginx、沒把共用設定攤到每個 client、沒標控制面）。但有**兩組**的性質不同——它們是**作者的出處本身難以被第三方複驗**：
...
**已補（§3.8）**：每一條改引 `src/frontend/main.go` 的 `*_SERVICE_ADDR` 宣告與 manifest 的 env 值，架構圖不再是任何一條的唯一出處。**而且這件事後來又多了一個代價**：追查工具的 recall 時發現架構圖漏畫了 `shoppingassistantservice`，照圖標註的 truth 也跟著漏了一條真的邊（§1.8）。**「架構圖」可以留著當補充，但不能是唯一的出處。**
...
**這是 truth 裡證據最弱的一組**：它可複驗（grep `Service.PERSISTENCE` 數引用次數），但複驗的人要先讀懂 `loadBalanceRESTOperation` 的分派機制才知道那個 enum 代表一個呼叫目標。論文要這樣寫：**這 7 條的等級不等於其他條**，它們靠的是對框架機制的理解，而不是一行可以指出來的字串。**已補（§3.8）**：出處改成第一個實際呼叫點，並查出 `webui → auth` 的業務呼叫其實在共用模組裡——跨兩層才看得出來。

**兩組加起來 16 條**——online-boutique 的 15 條（當時 16 條中的 15 條）與 TeaStore 的 7／13 條（非變體），佔七個專案 140 條非變體參考邊的 **11.4%**。 這就是「第二標註者」這個方法的真正價值——它一條邊都沒推翻，但它精準指出了**哪些邊的出處撐不起第三方複驗**。
...
4. **工具多畫的 35 條沒有逐條找第三方出處。** 它們在報告裡只列不評分。
