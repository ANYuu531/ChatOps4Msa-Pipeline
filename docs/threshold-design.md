...
> **2026-09-14 第一輪（範例標註，不算成績）**：BoA greenfield archive、80 段、有段落向量；15 題只有 11 題對得到段落，且 `code extraction` 一個子字串就命中 16 段，數字不可用。
> 參考方向而已：RRF@8 Recall 0.674、embedding@8 0.691、BM25@8 0.415；Hit@8 分別為 0.909／0.909／0.545。
> 標註檔已依 `archive-headings.txt` 重寫（看 heading 清單訂的，沒看每題名次），第二輪的數字才填進下表。
