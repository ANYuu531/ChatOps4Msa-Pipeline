...
| 部分圖節點上限 | 20 | `Graph/SubgraphExtractor.java:43` | 部分圖最多 20 節點 | 聊天訊息裡看不清（train-ticket 53 節點就是反例） | 流程被截 | **文獻上限（[Ghoniem 2005](https://doi.org/10.1057/palgrave.ivs.9500092)：超過 20 個頂點，節點連結圖在多數任務輸給矩陣）＋ 真圖量測** | **已做，第 8 節**（原本寫「工程估算／不需要」，2026-09-18 補） |
...
> - **外部 20 個真實系統**：MicroDepGraph 資料集（Rahman, Panichella & Taibi, *A Curated Dataset of Microservices-Based Systems*, SattoSE 2019，[CEUR-WS Vol-2520](https://ceur-ws.org/Vol-2520/paper1a.pdf)；資料集 [clowee/MicroserviceDataset](https://github.com/clowee/MicroserviceDataset)；LGPL-3.0，已收進 `src/test/resources/graphs/microdepgraph/`，出處與注意事項見該目錄的 `SOURCE.md`）。**加這批的原因**：只用自家兩張圖時，兩張的直徑都是 3，任何 ≥3 的跳數上限切出來都一樣，數字無法被驗證（2026-09-18 第一版就卡在這裡）。
...
- **節點**：取「讓 ≥90% 的子圖不被截斷」的最小上限，且**不得超過 20**——因為 Ghoniem, Fekete & Castagliola（*Information Visualization* 4(2), 2005，[doi:10.1057/palgrave.ivs.9500092](https://doi.org/10.1057/palgrave.ivs.9500092)）的受控實驗結論是「超過二十個頂點，矩陣式呈現在多數任務上勝過節點連結圖；只有路徑尋找（path finding）始終站在節點連結圖這邊」。部分圖要讀的正是路徑，所以形式選節點連結圖是對的，20 是該實驗給的門檻。
  **2026-09-22 補充**：這個比較在 Okoe, Jianu & Kobourov（IEEE TVCG 2019，[doi:10.1109/TVCG.2018.2865940](https://doi.org/10.1109/TVCG.2018.2865940)）有一次大規模重做——群眾外包、更廣的任務集、更大的真實網路（其中一個 258 節點 1090 邊）。結論是**節點連結圖在連通性與記憶類任務較好，矩陣在共同鄰居與群集類任務較好**，這**支持**我們選的形式，但它**沒有重述「二十個頂點」這個單一門檻**。因此本文件與論文一律把 20 描述為「Ghoniem et al. 2005 受控實驗的門檻」，**不寫成「公認的可讀性上限」**。

  > **2026-09-25 重寫這個上限的論證**，回應反饋「要找認知負荷相關的論文」與「我們的結果可以到 21，為什麼選 20」。完整版在 **`docs/graph-readability-cognitive-load.md`**（文獻、每一篇支持到哪裡、以及 20 與 21 到底在選什麼）。摘要三點：
  >
  > 1. **我們主張的不是「超過 20 就理解變弱」。** 那個命題要靠我們自己的使用者實驗才能講，而我們沒有做。我們主張的是：部分圖需要一個**安全閥**，它的刻度取自他人受控實驗的門檻（Ghoniem 2005），並由我們的語料驗證它幾乎不咬。措辭：✅「受控實驗的門檻」「文獻安全區內的保守值」「安全閥」；❌「公認的可讀性上限」「認知負荷上限」。
  > 2. **認知負荷的直接實證**：Yoghourdjian 等人（IEEE TVCG 27(2):1677–1687, 2021，[doi:10.1109/TVCG.2020.3030459](https://doi.org/10.1109/TVCG.2020.3030459)）用**正確率、反應時間、EEG、瞳孔、心率變異度**測 25–175 個節點的**最短路徑**任務（和部分圖同一類任務），發現**高密度圖超過 50 個節點、低密度圖超過 100 個節點**時受試者顯著困難，而且認知負荷升到一定難度後**反而下降**（放棄了）。這支持「20 夠小」，**不支持「21 就不行」**——照這篇，我們的上限甚至偏保守。同一批作者的綜述（*Visual Informatics* 2(4):264–282, 2018，[doi:10.1016/j.visinf.2018.12.006](https://doi.org/10.1016/j.visinf.2018.12.006)）則指出「大」是相對的，取決於資料複雜度、視覺複雜度與所用技術——所以沒有普適的節點數門檻，只能說清楚自己的情境（我們的情境是**聊天訊息裡的一張 PNG**）。
  > 3. **為什麼停在 20 而不是 21**：21 是我們**語料的極端值**（而且只出現在「一次餵 8 個種子」的壓力測試；真實問句點名 1–2 個服務，子圖 p90 只有 9–10），拿語料的最大值當上限就是對這批資料過擬合，換一個系統立刻又破。這個參數在 15–25 之間只影響 242 組裡的 1 組，**它不需要被調準**；真正該補的是**讓截斷看得見**——已實作（見 8.5 第 6 點）。
...
**放寬跳數幾乎不影響圖的大小**：對每一組兩個服務的問句、每一個跳數上限切出子圖，中位數 6 個節點、p90 **12** 個節點、最大 16 個節點，從 1 跳到 12 跳**這三個統計量完全一樣**。

但要注意這張表的每一列只統計「該跳數連得到的配對」（1 跳 436 組、2 跳 543 組、3 跳以上 568 組），**各列的母體不同**，所以它本身不足以證明「同一個問句放寬跳數不會變大」。`HopCostProbeTest` 把配對固定、只切換要不要納入連接路徑，逐組比對：

| 兩個種子的最短距離 | 配對數 | 尺寸變大的 | 總共多出的節點 |
|---|---|---|---|
| 1 跳 | 436 | 0（0.0%） | 0 |
| 2 跳 | 107 | **0（0.0%）** | 0 |
| 3 跳 | 25 | **25（100%）** | **25（每組各 +1）** |

所以正確的說法是**代價接近零但不是零**：568 組裡有 **25 組（4.4%）各多 1 個節點**，其餘 543 組完全不變；median／p90／max 不動，是因為 4.4% 的 +1 移不動這三個統計量。

機制在 `SubgraphExtractor` 的兩個步驟。鄰居那一步在多種子時只加兩種節點：「種子呼叫的資料儲存／佇列／外部主機」與「呼叫種子的人」。所以連接路徑 `a → x → b` 的中間節點 x 是 b 的呼叫者，**鄰居步驟本來就會加**，路徑不多帶任何東西；而 `a → x → y → b` 的兩個中間節點裡，y 是 b 的呼叫者會被加到，x 是 a 的非終端被呼叫者、多種子時不加，**只有這個 x 要靠路徑帶進來**。一句話：**一個節點只有在「離每個種子都超過一跳」時才需要靠路徑**，而這種節點只存在於長度 ≥ 3 的路徑中間；本語料的直徑就是 3，所以每組最多只會有一個。
...
3. 不設上限也不行：Alibaba 20 000 個微服務、100 億筆呼叫追蹤的研究（Luo et al., *Characterizing Microservice Dependency and Performance: Alibaba Trace Analysis*, SoCC 2021，[doi:10.1145/3472883.3487003](https://doi.org/10.1145/3472883.3487003)；追蹤資料 [alibaba/clusterdata](https://github.com/alibaba/clusterdata)）顯示**呼叫深度在 3 附近達到高峰，但有超過 4% 的呼叫圖深度超過 10**。真實大型系統有更深的尾巴；若沒有上限，那條尾巴會把兩個其實不相干的服務用一條 10 跳的路徑接成「一條流程」。4 跳＝涵蓋常見情況、拒絕長尾。
...
1. **4 跳＝實測最大深度 + 1，而且放寬的代價接近零。** 依據在 8.3：22 個真實系統的直徑最大就是 3；圖的大小對跳數幾乎不敏感（p90 一路都是 12 個節點；逐組比對只有 4.4% 的問句各多 1 個節點）。所以 4 不是隨手取的，而是「沒有任何真實系統會被它切斷，且再放寬也不會讓圖變難讀」。上限仍要存在，是為了 Alibaba 追蹤裡那 4% 深度超過 10 的長尾。
...
5. **20 與 21 之間，選的不是準確度而是原則（2026-09-25 補）。** 上限設 21 的話 242 組一組都不會被截，設 20 會截掉 1 組，所以差別可量化為 1／242；而 21 的唯一依據是「我們這次枚舉出來的最大值」。**注意這 242 組是程式窮舉的（滑動視窗），不是問句的機率樣本**，所以本節的 p90／最大值不能讀成「多少比例的問句會被截」——這一點 2026-09-25 前的版本寫得不夠清楚。**拿枚舉的極端值當上限等於宣告「上限＝目前算過最大的那個切片」**，換一個系統（train-ticket 的閘道有 46 條路由）立刻又破，而且破的時候沒有原則可以決定新值。20 有出處，且落在認知負荷實驗測到的困難區（高密度 50 個節點）之下——它是保守的安全閥，不是我們量到的極限。
   問句實際有多寬，唯一的樣本是路由 hold-out 的 102 句：58% 一個節點都沒點名、32% 點 1 個、9% 點 2 個、1% 點 3 個，**最寬 3 個，沒有一句到 8 個**（`HoldoutMentionCountTest`，可重跑）。**但那 102 句是作者寫的**，所以它是作者的問句分布而不是使用者的；真實使用者問句的樣本我們還沒有（見 8.6）。完整論證與文獻在 `docs/graph-readability-cognitive-load.md`。
6. **既然差別只是「那 1 組會不會被截」，補的就是讓截斷看得見（2026-09-25 已實作）。**
   - 截斷寫進**圖本身的標題**：`… · 20 of 23 nodes shown, limit 20`（`ReportQaService.partialGraphTitle`）。理由：圖會被複製到簡報或論文，訊息裡的說明不會跟著走。
   - 訊息裡的說明**點名**被省略的節點（最多 5 個）而不只報數量，讀者可以直接追問。
   - 給模型的文字版本本來就列出全部被省略的節點（`GraphQueryEngine.subgraph`），不變。
   - 測試：`PartialGraphQaTest.aCutSliceSaysSoOnThePictureAndNamesWhatIsMissing`。
7. **微服務依賴圖比想像中平。** 22 張圖裡有 9 張直徑只有 1、11 張是 2、2 張是 3。原因是這類系統多半是「閘道 ＋ 一層業務服務 ＋ 共用基礎設施」的星狀結構，不是長鏈。這件事本身值得寫進論文：**部分圖要解的問題其實是「寬」（一個服務的鄰居太多）而不是「深」**，所以節點上限才是那個會真正生效的參數（8.4 顯示它真的咬到了一次），跳數上限則是防長尾用的。
...
- 更深的真實系統只有**二手數字**（[Alibaba SoCC 2021](https://doi.org/10.1145/3472883.3487003) 的追蹤研究），我們沒有自己跑過那份 25 GB 的追蹤資料。要自己驗證，最省的做法是在機器 A／B 上把 train-ticket 真的部署起來、用 runtime 層產生一張有呼叫鏈的圖再重跑本實驗。
...
- 起點集合是程式列舉出來的（單點、前六個成員的兩兩組合、3／4／6／8 個成員的滑動視窗），**不是問句的機率樣本**，所以 8.4 的 p90 與最大值不能解讀成「多少比例的問句會被截」。8 這個上限來自 `GraphQuery` 的種子上限，是工程值。
- **「問句有多寬」的三個資料來源全部經過作者的手**：hold-out 的 102 句（作者寫的，量到最寬 3 個節點）、242 組起點集合（程式窮舉）、`flows.tsv` 的流程成員（作者依官方文件標註）。真實使用者問句的樣本只能來自已部署實例的 thread log（`graph query plan (router …)`），**尚未收集**——這是 2026-09-25 反饋指出的缺口。因此 8.5 第 5 點的主要理由寫成不依賴問句分布的形式。
- 可讀性只引文獻，**沒有做我們自己的使用者實驗**；20 是別人受控實驗的閾值，不是對 DepWeaver 圖的實測。因此「超過 20 個節點，讀者的理解會變弱」這個命題我們**不主張**（2026-09-25：老師原本以為那是我們的方向）。要主張它，最小可行設計是同一條流程分別畫成 10／20／30 個節點、8–10 位受試者、量正確率與時間與 NASA-TLX——列為後續工作，見 `docs/graph-readability-cognitive-load.md` §7。
- 引用的文獻與我們的情境有系統性差距：受控實驗用抽象網路、桌機螢幕、專門的佈局演算法；我們的圖是**聊天訊息裡的 PNG**、讀者只看幾秒、圖是分層有向圖。方向上對我們有利（該更保守），但那是推論不是量測。
...
**這套評分的三個界線（2026-09-23 補，之前只寫了第三個）**：

1. **它量的是「點名了哪些服務」，不是「說法對不對」。**一個把該點的節點全點對、但對它們的描述錯誤的回答（例如把量測到的邊說成沒量到）照樣拿 1.000。所以分數低一定有問題，**分數高只代表召回了正確的節點集合**，不能當成答案品質的充分條件。論文要這樣寫。
2. **否定句只在 gold 端過濾，answer 端沒有。**`goldFrom()` 會丟掉 `- no …`／`Query:` 等陳述不存在的行，但 `score()` 對模型回答直接呼叫 `namedNodes()`。理論上「A 不會呼叫 B」會被算成答對 A、B。**2026-09-23 逐題檢查三輪輸出，實際出現的否定句都是本來就該點名的正確陳述（「accounts-db 掛掉會影響誰」「尚未觀察到執行期連線」），沒有一題因此虛增分數**；而最容易中招的「答案是沒有」題型本來就不自動計分。仍列為已知缺陷。
3. `offGraphNames()` 的排除清單（`DSL_WORDS`）是手動維護的，加字是為了擋英文連字詞誤判，但同一個機制也可能擋掉真正的編造名。
