...
> - 第 5 點「top-k 要繼續測試更多的」→ 第 4.3 節換掉選 k 規則、網格擴到語料全長（待機器 B 重跑）
...
| 檢索 top-k | **16**（原 8） | `application.properties` 的 `dependency.qa.top-k`、`ReportQaService.java:110` | 放進 context 的段落數上限 | context 變長、雜訊段落稀釋關鍵段、成本增加 | 關鍵段落進不了 context → 模型說「報告沒有」或只憑圖回答 | 小型消融實驗（第 4.4 節，2026-09-14） | 已做，第 4 節 |
...
**第二版（2026-09-18，回應反饋第 5 點，寫在重跑之前）**——`ChunkRetrieverTopKAblationTest.selectK()`：

1. 只考慮**平均 context 字數 ≤ 段落預算 75%（18 000 字）**的 k；超過就等於把預算花在檢索，事實表與對話歷史會被擠掉。
2. 在這些 k 裡，取**每多一段的邊際 Recall 增益首次低於 0.01 的前一個 k**（曲線的膝點；0.01 約等於 15 題標註裡每 7 段才多對一題）。
3. 若網格內曲線始終沒有平掉，取預算內最大的 k，並在 md 裡註明「曲線尚未平坦」。
...
### 4.4 結果（2026-09-14 機器 B，第二輪標註；網格只到 16）

> **這一輪的結論只在 k ≤ 16 的範圍內成立**，選出 16 是碰到網格上限。依第 4.3 節第二版規則與更寬的網格重跑，是機器 B 待辦（步驟 7.2）；重跑後這一節改為第三輪，本表保留作為對照。

> **第一輪（範例標註，不算成績）**：15 題只有 11 題對得到段落，且 `code extraction` 一個子字串就命中 16 段，數字不可用。
> 標註檔隨後依 `archive-headings.txt` 重寫（看 heading 清單訂的，沒看每題名次），下表是第二輪。

- archive：`GoogleCloudPlatform/bank-of-anthos`，greenfield，80 段，有段落向量（`text-embedding-3-small`）
- 有效標註題數：15／15

| 模式 | 指標 | k=2 | 4 | 6 | 8 | 10 | 12 | 16 |
|---|---|---|---|---|---|---|---|---|
| BM25 | Recall@k | 0.228 | 0.283 | 0.297 | 0.397 | 0.410 | 0.482 | 0.529 |
| BM25 | Hit@k | 0.400 | 0.400 | 0.467 | 0.600 | 0.600 | 0.667 | 0.667 |
| embedding | Recall@k | 0.494 | 0.591 | 0.619 | 0.638 | 0.683 | 0.736 | 0.780 |
| embedding | MRR@k | 0.767 | 0.789 | 0.802 | 0.802 | 0.802 | 0.802 | 0.802 |
| RRF（正式） | Recall@k | 0.478 | 0.544 | 0.556 | 0.605 | 0.627 | 0.702 | **0.807** |
| RRF（正式） | MRR@k | 0.733 | 0.733 | 0.747 | 0.755 | 0.755 | 0.755 | 0.755 |
| RRF（正式） | Hit@k | 0.800 | 0.800 | 0.867 | 0.933 | 0.933 | 0.933 | 0.933 |
| RRF（正式） | 平均 context 字數 | 1399 | 2955 | 4692 | 6447 | 8563 | 10549 | 14421 |

**套用 4.3 的準則**：RRF 最高 Recall 0.807（k=16）；95% = 0.767；最小達到的 k = 16。它與 k=8 的差距 0.807 − 0.605 = 0.20，大於一題（約 0.07），**所以 top-k 由 8 改為 16**。

**解讀**

1. **k 影響的是「湊齊」，不是「找到」。** RRF 的 MRR 與 Hit 在 k=8 就飽和（0.755、0.933）：第一個相關段落幾乎都在前 8 名。k 從 8 到 16 增加的是 Recall，也就是需要多段證據的問題（Collection Status 有 4 段、資料庫證據分散在第 5 節與 persistence 段）能拿到更完整的一組。
2. **代價在預算內。** k=16 平均 14.4k 字，仍低於 24k 的段落預算，預算沒有變成實際的截斷點；每題大約多 2–3k token。
3. **BM25 單獨用明顯不夠。** k=8 時 Recall 0.397、Hit 0.600；中文問句對英文報告、換句話說的問題，BM25 對不上。
4. **RRF 沒有全面勝過 embedding 單獨用。** k ≤ 12 時 embedding 單獨略高（例如 k=8：0.638 對 0.605），k=16 時 RRF 反超（0.807 對 0.780），MRR 則 embedding 一路較高。只有一份 archive、15 題，**不據此拿掉 BM25**：BM25 對服務 id、file:line 這類精確字串的價值，這組偏向語意的問題測不太出來，且 embedding 打不到時它是唯一的退路。
5. **k=16 是網格上限。** Recall 在 16 仍在上升，更大的 k 可能更高；但已超過一半的段落預算，且 MRR 不再變，先停在 16。
6. **標註的界線**：`docs+code notes` 的 11 段沒有標題、無法用 heading 標註，答案在那裡的問題會被低估；這份是 greenfield，沒有流量與 k8s 階段。
